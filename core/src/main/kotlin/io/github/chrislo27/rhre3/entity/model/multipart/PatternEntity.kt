...
class PatternEntity(remix: Remix, datamodel: Pattern)
    ...
    init {
        ...
        this.bounds.width = internal
                .maxByOrNull { it.bounds.x + it.bounds.width }?.run { this.bounds.x + this.bounds.width - this@PatternEntity.bounds.x } ?: error("Nothing in internal cache")
        this.bounds.height = internal
                .maxByOrNull { it.bounds.y + it.bounds.height }?.run { this.bounds.y + this.bounds.height - this@PatternEntity.bounds.y } ?: error("Nothing in internal cache")
    }
