...
class InfoScreen(m: Main) : NewUIScreen(m) {
	...
	private val patternCount: Int by lazy {
		OldGameRegistry.gameList.flatMap(Game::patterns).filter { !it.autoGenerated }.count()
	}

	private val soundCueCount: Int by lazy {
		OldGameRegistry.gameList.map { it.soundCues.size }.sum()
	}
	...
	override fun render(delta: Float) {
		...
		val stats: String = Localization.get("info.stats", "${OldGameRegistry.gameList.size}", "$patternCount",
											 "$soundCueCount")
