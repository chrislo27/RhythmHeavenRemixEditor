...
//					Main.logger.debug("${result.game.id} finished in ${result.timeMs} ms")
                    ...
                    result.game.soundCues.forEach { sc ->
                        if (cueMap.containsKey(sc.id)) {
                            errors += GameParseError(result.game.id,
                                                     "Duplicate sound cue ID ${sc.id} (already assigned in ${cueMap[sc.id]?.gameID})")
                        } else {
                            cueMap[sc.id] = sc
                        }
                        ...
                        sc.deprecated.forEach {
                            if (cueMapDeprecations.containsKey(it)) {
                                errors += GameParseError(result.game.id,
                                                         "Duplicate deprecation of sound cue ID $it in ${sc.id} (already assigned to ${cueMapDeprecations[it]?.id})")
                            } else {
                    ...
                    result.game.patterns.forEach { pattern ->
                        if (patternMap.containsKey(pattern.id)) {
                            errors += GameParseError(result.game.id,
                                                     "Duplicate pattern ID ${pattern.id} (already assigned in ${patternMap[pattern.id]?.gameID})")
                        } else {
                            patternMap[pattern.id] = pattern
                        }
                        if (patternMapDeprecations.containsKey(
                                pattern.id) && patternMapDeprecations[pattern.id]!!.id != pattern.id) {
                            errors += GameParseError(result.game.id,
                                                     "Duplicate pattern ID ${pattern.id} (already assigned in ${patternMap[pattern.id]?.gameID})")
                        } else {
                        ...
                        pattern.deprecated.forEach {
                            if (patternMapDeprecations.containsKey(it)) {
                                errors += GameParseError(result.game.id,
                                                         "Duplicate deprecation of pattern ID $it in ${pattern.id} (already assigned to ${patternMapDeprecations[it]?.id})")
                            } else {
