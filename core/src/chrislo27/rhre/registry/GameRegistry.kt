				...
				gameList.forEach { game ->
					game.soundCues.forEach { sc ->
						...
						sc.deprecated.forEach {
							if (checkedDeprecations[it] != null) {
								Main.logger.warn("Duplicate deprecation $it in sound cue ${sc.id} (first existed in ${checkedDeprecations[it]})")
								warningCount++
							} else {
					...
					game.patterns.forEach { pat ->
						...
						pat.deprecated.forEach {
							if (checkedDeprecations[it] != null) {
								Main.logger.warn("Duplicate deprecation $it in pattern ${pat.id} (first existed in ${checkedDeprecations[it]})")
								warningCount++
							} else {
