...
//		val coroutinesToUse = coroutinesToUse
		...
		fun loadGameDefinition(gameDef: String, id: Int = -1) {
		...
		val coroutinesActuallyUsed: Int

//		if (coroutinesToUse <= 0) {
			coroutinesActuallyUsed = 0

			// games from games.json
			games.forEach { loadGameDefinition(it) }

			// custom sounds
			customFolders.forEach(::loadCustomSoundFolder)
//		} else {
//			// TODO allocate coroutines
//			val coroutines = mutableListOf<Deferred<*>>()
//
//			// games from games.json
//			games.forEachIndexed { i, gameDef ->
//				coroutines.add(async(CommonPool) {
//					loadGameDefinition(gameDef, i)
//				})
//			}
//
//			// custom sounds
//			customFolders.forEach { fh ->
//				coroutines.add(async(CommonPool) {
//					loadCustomSoundFolder(fh)
//				})
//			}
//
//			// TODO await stuff
//			runBlocking {
//				coroutines.forEachIndexed { i, it ->
//					val n = System.nanoTime()
//					it.join()
//					println("$i took " + ((System.nanoTime() - n) / 1000000.0) + " ms")
//				}
//			}
//
//			coroutinesActuallyUsed = coroutines.size
//		}
		...
		Main.logger.info(
				"Loaded " + this.gameList.size + " games (" +
						...
						" using $coroutinesActuallyUsed coroutine(s)")
	}
