...
class EditorStageSetup(private val screen: EditorScreen) {
    ...
    private fun create() {
        ...
        run {
            val inspections = object : ImageButton(stage, palette,
                                                   TextureRegion(AssetRegistry.getTexture("ui_inspections"))) {
                override fun render(batch: SpriteBatch, alpha: Float) {
                    getPalette().labelFont.data.setScale(0.5f)
                    super.render(batch, alpha)
                    getPalette().labelFont.data.setScale(1f)
        ...
        run {
            val present = object : ImageButton(stage, palette,
                                               TextureRegion(AssetRegistry.getTexture("ui_presentation_mode"))) {
