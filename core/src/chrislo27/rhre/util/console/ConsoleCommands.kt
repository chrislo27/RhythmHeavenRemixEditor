    ...
    fun handle(main: Main, command: String, args: List<String>): Boolean {
        ...
        return when (command.toLowerCase(Locale.ROOT)) {
            ...
            "dumpids" -> {
                ...
                if (writeWithoutPrompt || (readLine() ?: throw IllegalArgumentException("Got null for yes/no")).equals(
                        "y", ignoreCase = true)) {
...
""")
    ...
    private fun rename(original: String, new: String) {
        val obj: GameObject = JsonHandler.fromJson(
                Gdx.files.local("sounds/cues/$original/data.json").readString("UTF-8"))
